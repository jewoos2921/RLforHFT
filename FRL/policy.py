...
from FRL.distribution import Choose, Constant, Distribution, FiniteDistribution
...
class RandomPolicy(Policy[S, A]):
    """
    A policy that randomly selects one of several specified policies each action
    Given the right inputs, this could simulate things like e-greedy.
    """
...
class Always(DeterministicPolicy[S, A]):
    """
    A constant Policy: always return the same (specified) action for every possible state.
    """
...
class FiniteDeterministicPolicy(FinitePolicy[S, A]):
    ...
    def __init__(self, action_for: Mapping[S, A]):
        ...
        super().__init__(
            policy_map={s: Constant(a) for s, a in action_for.items()})
