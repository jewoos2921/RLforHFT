...
from FRL.function_approx import Gradient
from FRL.markov_decision_process import NonTerminal
import FRL.markov_process as mp
import numpy as np
from FRL.approximate_dynamic_programming import ValueFunctionApprox, extended_vf
...
def lambda_return_prediction(
        ...
        gamma: float,
        lambd: float) -> Iterator[ValueFunctionApprox[S]]:
    ...
    for trace in traces:
        ...
        for t, tr in enumerate(trace_seq):
            for i, partial in enumerate(partials):
                partial.append(
                    partial[-1] + gp[t - i] * (tr.reward - func_approx(tr.state)) +
                    (gp[t - i] * gamma * extended_vf(func_approx, tr.next_state)
                     if t < len(trace_seq) - 1 else 0.))
                weights[i].append(
                    weights[i][-1] * lambd if t < len(trace_seq) else lp[t - i])
...
def td_lambda_prediction(traces: Iterable[Iterable[mp.TransitionStep[S]]],
                         approx_0: ValueFunctionApprox[S],
                         gamma: float,
                         lambd: float) -> Iterator[ValueFunctionApprox[S]]:
    ...
    for trace in traces:
        ...
        for step in trace:
            ...
            func_approx = func_approx.update(
