...
from FRL.distribution import Categorical, Distribution, FiniteDistribution, SampledDistribution
...
class MarkovProcess(ABC, Generic[S]):
    ...
    def transition(self, state: NonTerminal[S]) -> Distribution[State[S]]:
        """Given a state of the process, returns a distribution of the next states.
        Retruning None means we are in a terminal state.
        """
        pass

    def simulate(self, start_state_distribution: Distribution[NonTerminal[S]]) -> Iterable[State[S]]:
        """Run a simulation trace of this Markov process, generating the states visited during the trace.
        This yields the start state first, then continues yielding subsequent states or until we hit a termianl state.
        """
...
class FiniteMarkovProcess(MarkovProcess[S]):
    """A Markov Process with finite states space.
    ...
    def get_stationary_distribution(self) -> FiniteDistribution[NonTerminal[S]]:
...
class TransitionStep(Generic[S]):
    ...
    def add_return(self, y: float, return_: float) -> ReturnStep[S]:
        """Given a y and return from 'next state', this annotates the
         transition with a return for 'state'."""
        return ReturnStep(self.state, self.next_state, self.reward,
...
class MarkovRewardProcess(MarkovProcess[S]):
    ...
    def transition_reward(self, state: NonTerminal[S]) -> Distribution[Tuple[State[S], float]]:
        """Given a state of the process, returns a distribution of the next states and
        reward from transitioning between the states.
        """
        pass

    def simulate_reward(self, start_state_distribution: Distribution[NonTerminal[S]]) -> Iterable[TransitionStep[S]]:
        """ Siulate the MRP, yielding an iterable of (state, next state, reward)for each simpled transition."""
    ...
    def reward_traces(self, start_state_distribution: Distribution[NonTerminal[S]]) -> Iterable[
...
class FiniteMarkovRewardProcess(FiniteMarkovProcess[S], MarkovRewardProcess[S]):
    ...
    def __init__(self, transition_reward_map: Mapping[S, FiniteDistribution[Tuple[S, float]]]):
        ...
        self.transition_reward_map = {
            NonTerminal(s): Categorical(
                {(NonTerminal(s1) if s1 in nt else Terminal(s1), r): p for (s1, r), p in v}
