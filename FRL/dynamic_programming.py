import operator
from typing import Mapping, Iterator, Tuple, Dict, TypeVar
import numpy as np

from FRL.distribution import Categorical, Choose
from FRL.iterate import converged, iterate
from FRL.markov_process import NonTerminal, State
from FRL.markov_decision_process import FiniteMarkovDecisionProcess, FiniteMarkovRewardProcess
from FRL.policy import FinitePolicy, FiniteDeterministicPolicy
...
def almost_equal_np_arrays(v1: np.ndarray,
                           v2: np.ndarray,
                           tolerance: float = DEFAULT_TOLERANCE) -> bool:
    """Return whether the two value functions as np.ndarray are withing
...
def evaluate_mrp_result(mrp: FiniteMarkovRewardProcess[S],
    ...
    v_star: np.ndarray = converged(
        evaluate_mrp(mrp, gamma), almost_equal_np_arrays
    )
    return {s: v_star[i] for i, s in enumerate(mrp.non_terminal_states)}


def greedy_policy_from_vf(mdp: FiniteMarkovDecisionProcess[S, A],
                          vf: V[S],
...
def policy_iteration(mdp: FiniteMarkovDecisionProcess[S, A],
    ...
    def update(vf_policy: Tuple[V[S], FinitePolicy[S, A]]) -> Tuple[V[S], FiniteDeterministicPolicy[S, A]]:
        ...
        policy_vf: V[S] = {mrp.non_terminal_states[i]:
                               v for i, v in enumerate(mrp.get_value_function_vec(gamma))} \
...
def almost_equal_vf_pis(
        ...
        x2: Tuple[V[S], FinitePolicy[S, A]]) -> bool:
...
def policy_iteration_result(
    ...
    return converged(policy_iteration(mdp, gamma), almost_equal_vf_pis)
...
def value_iteration(mdp: FiniteMarkovDecisionProcess[S, A],
    ...
    def update(vf: V[S]) -> V[S]:
        policy_vf: V[S] = {s: max(
            mdp.mapping[s][a].expectation(
                lambda s_r: s_r[1] + gamma * extended_vf(vf, s_r[0])
            ) for a in mdp.actions(s)) for s in vf}
        return policy_vf
...
def almost_equal_vfs(v1: V[S], v2: V[S], tolerance: float = DEFAULT_TOLERANCE) -> bool:
    """Return whether the two value function tables are withing the given tolerance of each other."""
    return max(abs(v1[s] - v2[s]) for s in v1) < tolerance


def value_iteration_result(mdp: FiniteMarkovDecisionProcess[S, A], gamma: float) -> \
        Tuple[V[S], FiniteDeterministicPolicy[S, A]]:
    opt_vf: V[S] = converged(value_iteration(mdp, gamma), almost_equal_vfs)
