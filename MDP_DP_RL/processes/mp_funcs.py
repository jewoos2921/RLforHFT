import numpy as np
from typing import Mapping, Set, Sequence, Tuple, Any, Callable
from random import choices
from operator import itemgetter
from scipy.stats import rv_discrete

from src.utils.gen_utils import memoize, is_approx_eq, sum_dicts, FlattenedDict
from src.utils.standard_typevars import S, A
from src.utils.standard_typevars import SSf, SAf, SASf, SATSff
...
@memoize
def get_actions_for_states(mdp_data: Mapping[S, Mapping[A, Any]]) -> Mapping[S, Set[A]]:
...
def get_lean_transitions(d: Mapping[S, float]) -> Mapping[S, float]:
    return {k: v for k, v in d.items() if not is_approx_eq(v, 0)}


def verify_transitions(states: Set[S],
                       tr_seq: Sequence[Mapping[S, float]]) -> bool:
...
def verify_policy(policy_data: SAf) -> bool:
    return all(is_approx_eq(sum(v.values()), 1.0) for _, v in policy_data.items())


def mdp_rep_to_mrp_rep1(mdp_rep: SASf, policy_rep: SAf) -> SAf:
    return {s: sum_dicts([{s1: p * v2 for s1, v2 in v[a].items()}
                          for a, p in policy_rep[s].items()]) for s, v in mdp_rep.items()}


def mdp_rep_to_mrp_rep2(mdp_rep: SAf,
                        policy_rep: SAf) -> Mapping[S, float]:
    return {s: sum(p * v[a] for a, p in policy_rep[s].items())
            for s, v in mdp_rep.items()}


def mdp_func_to_mrp_func1(mdp_rep: Callable[[S, A], Mapping[S, float]],
                          policy_func: Callable[[S], Mapping[A, float]]) -> Callable[[S], Mapping[S, float]]:
    def mrp_func1(s: S,
                  mdp_rep=mdp_rep,
...
def mdp_func_to_mrp_func2(mdp_rep: Callable[[S, A], float],
                          policy_func: Callable[[S], Mapping[A, float],]) -> Callable[[S], float]:
    def mrp_func2(s: S,
                  mdp_rep=mdp_rep,
                  policy_func=policy_func) -> float:
        s_dict = policy_func(s)
...
def get_rv_gen_func(prob_dict: Mapping[S, float]) -> Callable[[int], Sequence[S]]:
    outcomes, probabilities = zip(*prob_dict.items())
    rvd = rv_discrete(values=(range(len(outcomes)), probabilities))
    return lambda n, rvd=rvd, outcomes=outcomes: [outcomes[k] for k in rvd.rvs(size=n)]


def get_state_reward_gen_func(prob_dict: Mapping[S, float],
                              rew_dict: Mapping[S, float]) -> Callable[[], Tuple[S, float]]:
...
def get_state_reward_gen_dict(tr: SASf, rr: SASf) -> Mapping[S, Mapping[A, Callable[[], Tuple[S, float]]]]:
    return {s: {a: get_state_reward_gen_func(tr[s][a], rr[s][a]) for a, _ in v.items()} for s, v in rr.items()}


def get_epsilon_action_probs(action_value_dict: Mapping[A, float],
                             epsilon: float) -> Mapping[A, float]:
    max_act = max(action_value_dict.items(), key=itemgetter(1))[0]
    if epsilon == 0:
        ret = {max_act: 1}
    else:
        ret = {a: epsilon / len(action_value_dict) + (1. - epsilon if a == max_act else 0.) for a in
               action_value_dict.keys()}
    return ret


def get_softmax_action_probs(action_value_dict: Mapping[A, float]) -> Mapping[A, float]:
    aq = {a: q - max(action_value_dict.values()) for a, q in action_value_dict.items()}
    exp_sum = sum(np.exp(q) for q in aq.values())
    return {a: np.exp(q) / exp_sum for a, q in aq.items()}


def get_expected_action_value(action_value_dict: Mapping[A, float],
                              softmax: bool,
                              epsilon: float) -> float:
    av = action_value_dict
    ap = get_softmax_action_probs(av) if softmax else get_epsilon_action_probs(av, epsilon)
    return sum(ap.get(a, 0.) * v for a, v in av.items())


def get_sampling_func_from_prob_dict(prob_dict: Mapping[A, float]) -> Callable[[int], Sequence[A]]:
