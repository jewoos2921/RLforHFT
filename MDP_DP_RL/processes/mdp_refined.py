...
from src.processes.mdp import MDP
from src.processes.mdp_rep_for_adp_pg import MDPRepForADPPG
from src.processes.mdp_rep_for_rl_pg import MDPRepForRLPG
from src.processes.mdp_rep_for_rl_tabular import MDPRepForRLTabular
from src.processes.mp_funcs import (
    flatten_ssf_dict, flatten_sasf_dict,
    get_rv_gen_func, get_rv_gen_func_single,
    get_sampling_func_from_prob_dict,
    get_state_reward_gen_func, get_state_reward_gen_dict,
    mdp_rep_to_mrp_rep1, unflatten_ssf_dict, unflatten_sasf_dict)
from src.processes.policy import Policy
from src.processes.mrp_refined import MRPRefined
from src.utils.gen_utils import zip_dict_of_tuple, merge_dicts
from src.utils.standard_typevars import SAf, SASTff, SASf
...
class MDPRefined(MDP):
    ...
    def get_mrp_refined(self, pol: Policy) -> MRPRefined:
        ...
        flat_exp_rewards = merge_dicts(flat_rewards_refined, flat_transitions,
                                       lambda x, y: x * y)
        ...
        return MRPRefined({s: {s1: (v1, norm_rewards[s][s1])
                               for s1, v1 in v.items()}
                           for s, v in tr.items()}, self.gamma)

    def get_mdp_rep_for_rl_tabular(self) -> MDPRepForRLTabular:
        return MDPRepForRLTabular(state_action_dict=self.state_action_dict,
                                  terminal_states=self.terminal_states,
                                  state_reward_gen_dict=get_state_reward_gen_dict(self.transitions,
    ...
    def get_mdp_rep_for_rl_pg(self) -> MDPRepForRLPG:
        return MDPRepForRLPG(
            ...
            init_state_gen_func=get_rv_gen_func_single(
                {s: 1. / len(self.state_action_dict) for s in self.state_action_dict.keys()}),
            state_reward_gen_func=lambda s, a: get_state_reward_gen_func(self.transitions[s][a],
