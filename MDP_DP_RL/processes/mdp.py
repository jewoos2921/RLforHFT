...
from src.utils.gen_utils import zip_dict_of_tuple, is_approx_eq
from src.processes.mp_funcs import get_all_states, get_actions_for_states
from src.processes.mp_funcs import verify_mdp, get_lean_transitions
from src.processes.policy import Policy
from src.processes.det_policy import DetPolicy
from src.processes.mp_funcs import mdp_rep_to_mrp_rep1, mdp_rep_to_mrp_rep2
from operator import itemgetter
from src.processes.mrp import MRP
from src.processes.mp_funcs import get_rv_gen_func
from src.processes.mdp_rep_for_adp import MDPRepForADP
from src.utils.generic_typevars import A, S
...
class MDP(Generic[S, A]):
    def __init__(self,
        ...
        if verify_mdp(info):
            ...
            self.transitions: Mapping[S, Mapping[A, Mapping[S, float]]] = {s: {a: get_lean_transitions(v1)
                                                                               for a, v1 in v.items()}
                                                                           for s, v in d1.items()}
    ...
    def get_sink_states(self) -> Set[S]:
        return {k for k, v in self.transitions.items() if all(len(v1) == 1 and k in v1.keys() for _, v1 in v.items())}

    def get_terminal_states(self) -> Set[S]:
        sink = self.get_sink_states()
        return {s for s in sink if all(is_approx_eq(r, 0.0)
                                       for _, r in self.rewards[s].items())}
    ...
    def get_act_value_func_dict(self, pol: Policy) -> Mapping[S, Mapping[A, float]]:
        v_dict = self.get_value_func_dict(pol)
        return {s: {a: r + self.gamma * sum(p * v_dict[s1] for s1, p in self.transitions[s][a].items())
                    for a, r in v.items()}
                for s, v in self.rewards.items()}

    def get_improved_policy(self, pol: Policy) -> DetPolicy:
        q_dict = self.get_act_value_func_dict(pol)
        return DetPolicy({s: max(v.items(), key=itemgetter(1))[0] for s, v in q_dict.items()})

    def get_optimal_policy(self, tol=1e-4) -> DetPolicy:
        pol = Policy({s: {a: 1. / len(v) for a in v} for s, v in self.state_action_dict.items()})
    ...
    def get_mdp_rep_for_adp(self) -> MDPRepForADP:
        return MDPRepForADP(state_action_func=lambda s: self.state_action_dict[s],
                            gamma=self.gamma,
                            sample_states_gen_func=get_rv_gen_func(
                                {s: 1. / len(self.state_action_dict) for s in self.state_action_dict.keys()}),
