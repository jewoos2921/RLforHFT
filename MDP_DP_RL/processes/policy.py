...
from src.processes.mp_funcs import (verify_policy,
                                   get_epsilon_action_probs,
                                   get_softmax_action_probs)
...
class Policy(Generic[S, A]):
    ...
    def get_state_action_probability(self, state: S, action: A) -> float:
        return self.get_state_probabilities(state).get(action, 0.0)

    def edit_state_action_to_epsilon_greedy(self,
                                            state: S,
                                            action_value_dict: Mapping[A, float],
                                            epsilon: float) -> None:
        self.policy_data[state] = get_epsilon_action_probs(action_value_dict, epsilon)

    def edit_state_action_to_softmax(self, state: S, action_value_dict: Mapping[A, float]) -> None:
        self.policy_data[state] = get_softmax_action_probs(action_value_dict)
