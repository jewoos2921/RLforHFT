...
from src.processes.mp_funcs import get_all_states, verify_mp, get_lean_transitions
import numpy as np
from scipy.linalg import eig
from src.utils.standard_typevars import S, SSf
...
class MP(Generic[S]):
    def __init__(self, tr: SSf) -> None:
        if verify_mp(tr):
            ...
            self.transitions: SSf = {s: get_lean_transitions(v) for s, v in tr.items()}

        else:
            raise ValueError

    def get_sink_states(self) -> Set[S]:
        return {k for k, v in self.transitions.items() if len(v) == 1 and k in v.keys()}
