...
X = TypeVar('X')  # generic type for inputs
very_small_pos = 1e-6
...
class FuncApproxBase(ABC):
    ...
    @staticmethod
    def get_indicator_features_funcs(values: Set[X]) -> List[Callable[[X], float]]:
        ...
        for l in range(len(self.params)):
            ...
            if self.adam:
                ...
                self.params[l] -= self.learning_rate * self.adam_caches[0][l] / (
                    ...
                    1 - self.adam_decay2 ** self.time) / (1 - self.adam_decay1 ** self.time)
