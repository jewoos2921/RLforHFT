...
class LinearApprox(FuncApproxBase):
    def __init__(self,
        ...
        super().__init__(feature_funcs,
                         reglr_coeff, learning_rate, adam, adam_decay1, adam_decay2,
                         add_unit_feature)
    ...
    def get_func_eval_pts(self, x_vals_seq: Sequence[X]) -> np.ndarray:
        return np.dot(self.get_feature_vals_pts(x_vals_seq), self.params[0])
    ...
    def get_el_tr_sum_loss_gradient(self, x_vals_seq: Sequence[X],
        ...
        toepltiz_mat = get_decay_toeplitz_matrix(len(x_vals_seq), gamma_lambda)
        errors = self.get_func_eval_pts(x_vals_seq) - supervisory_seq
        func_grad = self.get_func_eval_pts(x_vals_seq)
        return [errors.dot(toepltiz_mat.dot(func_grad))]
    ...
    def get_el_tr_sum_objective_gradient(self, x_vals_seq: Sequence[X],
        ...
        return [factors.dot(toepltiz_mat.dot(np.diag(dObj_dOL).dot(features)))]
