...
from src.utils.standard_typevars import VFType, QFType, PolicyType
...
class OptBase(ABC):
    ...
    def get_optimal_value_func(self) -> VFType:
        ...
        return self.get_value_func(lambda s, pf=pf: lambda n, s=s, pf=pf: [pf(s)] * n)

    def get_optimal_act_value_func(self) -> QFType:
        pf = self.get_optimal_det_policy_func()
        return self.get_act_value_func(lambda s, pf=pf: lambda n, s=s, pf=pf: [pf(s)] * n)
