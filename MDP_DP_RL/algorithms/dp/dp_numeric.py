from src.algorithms.dp.dp_base import DPBase
...
from src.utils.standard_typevars import VFDictType
from src.processes.mdp import MDP


class DPNumeric(DPBase):
    """
    Class for performing dynamic programming on numeric MDPs.
    """
    ...
    def get_value_func_dict(self, pol: Policy) -> VFDictType:
        ...
        while epsilon >= self.tol:
            new_vf = {s: rew[s] + mo.gamma * sum(p * vf[s1] for s1, p in prob[s].items())
