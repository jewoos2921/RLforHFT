...
from src.algorithms.tabular_base import TabularBase
...
from src.algorithms.helper_funcs import get_uniform_policy, get_det_policy_from_qf_dict
from src.utils.generic_typevars import S, A
from src.utils.standard_typevars import VFDictType, QFDictType
...
class DPBase(TabularBase):
    ...
    def get_optimal_policy_vi(self) -> DetPolicy:
        ...
        mo = self.mdp_obj
        while epsilon >= self.tol:
            new_vf = {s: max(r + mo.gamma *
                             sum(p * vf[s1] for s1, p in mo.transitions[s][a].items())
                             for a, r in v.items())
                      for s, v in mo.rewards.items()}

            epsilon = max(abs(new_vf[s] - vf[s]) for s in mo.all_states)
            vf = new_vf
        pol = DetPolicy({s: max(((a, r + mo.gamma *
                                  sum(p * vf[s1] for s1, p in mo.transitions[s][a].items()))
                                 for a, r in v.items()), key=itemgetter(1))[0]
                         for s, v in mo.rewards.items()})
        return pol
