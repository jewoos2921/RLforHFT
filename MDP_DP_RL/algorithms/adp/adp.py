...
from src.algorithms.helper_funcs import get_uniform_policy_func
from src.algorithms.opt_base import OptBase
from src.algorithms.helper_funcs import get_epsilon_decay_func
from src.processes.mdp_rep_for_adp import MDPRepForADP
from src.algorithms.helper_funcs import get_soft_policy_func_from_qf
from src.algorithms.func_approx_spec import FuncApproxSpec
from src.func_approx.func_approx_base import FuncApproxBase
from src.processes.mp_funcs import mdp_func_to_mrp_func1, mdp_func_to_mrp_func2
from src.processes.mp_funcs import get_expected_action_value
from src.algorithms.helper_funcs import get_pdf_from_samples
from copy import deepcopy
from operator import itemgetter
import numpy as np
from src.utils.generic_typevars import S, A
from src.utils.standard_typevars import VFType, QFType
from src.utils.standard_typevars import PolicyType, PolicyActDictType
...
class ADP(OptBase):
    ...
    def __init__(self,
        ...
        self.epsilon_func: Callable[[int], float] = get_epsilon_decay_func(epsilon, epsilon_half_life)
        self.tol: float = tol
        self.fa: FuncApproxBase = fa_spec.get_vf_func_approx_obj()
        self.state_action_func: Callable[[S], Set[A]] = mdp_rep_for_adp.state_action_func
    ...
    def get_value_func_fa(self, polf: PolicyActDictType) -> VFType:
        ...
        sample_func = mo.sample_states_gen_func
        while epsilon >= self.tol:
            samples = sample_func(self.num_samples)
            values = [rew_func(s) + mo.gamma *
                      sum(p * self.fa.get_func_eval(s1) for s1, p in prob_func(s).items())
                      for s in samples]
            avg_grad = [g / len(samples) for g in self.fa.get_sum_loss_gradient(samples, values)]
    ...
    def get_act_value_func_fa(self, polf: PolicyActDictType) -> QFType:
        ...
        def state_func(s: S, mo=mo, v_func=v_func) -> Callable[[A], float]:
            def act_func(a: A, mo=mo, v_func=v_func) -> float:
                return mo.reward_func(s, a) + mo.gamma * sum(
                    p * v_func(s1) for s1, p in mo.transitions_func(s, a).items())
    ...
    def get_value_func(self, pol_func: PolicyType) -> VFType:
        return self.get_value_func_fa(
            lambda s: get_pdf_from_samples(
                pol_func(s)(len(self.state_action_func(s)) * ADP.NUM_SAMPLES_PER_ACTION)))

    def get_act_value_func(self, pol_func: PolicyType) -> QFType:
        return self.get_act_value_func_fa(
            lambda s: get_pdf_from_samples(pol_func(s)(len(self.state_action_func(s)) * ADP.NUM_SAMPLES_PER_ACTION)))
    ...
    def get_optimal_policy_func_pi(self) -> Callable[[S], A]:
        ...
        while eps >= self.tol:
            # Get the action-value function given the current policy
            qvf = self.get_act_value_func_fa(this_polf)

            # Ensure qvf is callable and valid
            if not callable(qvf):
                raise TypeError("Expected qvf to be callable")

            # Define the lambda function to access the Q-value
            def qf(sa):
                return qvf(sa[0])(sa[1])

            # Update the policy function using the soft policy derived from Q-function
            this_polf = get_soft_policy_func_from_qf(
                qf=qf,
                state_action_func=self.state_action_func,
                softmax=self.softmax,
            ...
            # Calculate the change in parameters and update eps
            new_params = deepcopy(self.fa.params)
            eps = ADP.get_gradient_max(
                [new_params[i] - p for i, p in enumerate(params)]
            )
        ...
        # Define the deterministic policy function
        def det_pol(s: S) -> A:
            return max(this_polf(s).items(), key=itemgetter(1))[0]
        ...
        while eps >= self.tol:
            ...
            values = [get_expected_action_value(
                {a: rew_func(s, a) + mo.gamma *
                    sum(p * self.fa.get_func_eval(s1) for s1, p in tr_func(s, a).items())
            ...
            new_params = deepcopy(self.fa.params)
            eps = ADP.get_gradient_max([new_params[i] - p for i, p in enumerate(params)])
            params = new_params
            iters += 1

        def deter_func(s: S, rew_func=rew_func, tr_func=tr_func) -> A:
            return max([(a, rew_func(s, a) +
                         sum(p * self.fa.get_func_eval(s1) for s1, p in tr_func(s, a).items()))
                        for a in self.state_action_func(s)],
                       key=itemgetter(1))[0]
...
if __name__ == '__main__':
    from src.processes.mdp_refined import MDPRefined
