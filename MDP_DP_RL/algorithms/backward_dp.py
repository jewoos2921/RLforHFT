...
from src.utils.standard_typevars import S, A, SASTff
...
class BackwardDP(Generic[S, A]):
    ...
    def verify_data(transitions_rewards: Sequence[SASTff],
        ...
        while valid and i < time_len:
            ...
            states = set((transitions_rewards[i + 1] if i < time_len - 1 else terminal_opt_val).keys())
    ...
    def get_vf_and_policy(self) -> Sequence[Mapping[S, Tuple[float, A]]]:
        ...
        for tr in self.transitions_rewards[::-1]:
            vf_pol = {s: max(
                [(sum(p * (r + self.gamma * vf_pol[s1][0]) for s1, (p, r) in d1.items()), a)
